...
export enum LegacyPersonalBestType {
  // Theatre of Blood splits.
  // All distinct ToB splits must have consecutive integer values, in the order
  // [entry mode, regular, hard mode] to allow for easy indexing on mode.
  TOB_ENTRY_CHALLENGE = 0,
  ...
  TOB_ENTRY_NYLO_BOSS_SPAWN = 15,
  TOB_REG_NYLO_BOSS_SPAWN = 16,
  TOB_HM_NYLO_BOSS_SPAWN = 17,
  TOB_NYLO_BOSS_SPAWN = TOB_ENTRY_NYLO_BOSS_SPAWN,
...
const genericTobPbTypes = [
  LegacyPersonalBestType.TOB_CHALLENGE,
  LegacyPersonalBestType.TOB_OVERALL,
  LegacyPersonalBestType.TOB_MAIDEN,
  LegacyPersonalBestType.TOB_BLOAT,
  LegacyPersonalBestType.TOB_NYLO_ROOM,
  LegacyPersonalBestType.TOB_NYLO_BOSS_SPAWN,
  LegacyPersonalBestType.TOB_NYLO_BOSS,
  LegacyPersonalBestType.TOB_SOTETSEG,
  LegacyPersonalBestType.TOB_XARPUS,
  LegacyPersonalBestType.TOB_VERZIK_ROOM,
  LegacyPersonalBestType.TOB_VERZIK_P1,
  LegacyPersonalBestType.TOB_VERZIK_P2,
  LegacyPersonalBestType.TOB_VERZIK_P3,
];
...
export function tobPbForMode(
  genericTobPb: LegacyPersonalBestType,
  mode: ChallengeMode,
...
/**
 * Return the mode-specific version of a generic ToB split type.
 *
 * This must **only** be called with a generic ToB split type, e.g.
 * `TOB_NYLO_BOSS_SPAWN`, and not with a mode-specific type like e.g.
 * `TOB_REG_NYLO_BOSS_SPAWN`.
 *
 * @param genericTobSplit The generic ToB split type.
 * @param mode Raid mode.
 * @returns Split type for the raid mode.
 */
export function tobSplitForMode(
  genericTobSplit: SplitType,
  mode: ChallengeMode,
): SplitType {
  if (process.env.NODE_ENV !== 'production') {
    if (!genericTobSplits.includes(genericTobSplit)) {
      throw new Error(
        `Invalid generic ToB personal best type: ${genericTobSplit}`,
      );
    }
  }
