...
export class EventConsolidator {
  ...
  private consolidateTicks(): void {
    for (let m = 0; m < this.mergedTicks.length; m++) {
      ...
      if (hasBase && hasTarget) {
        ...
        this.mergeNpcAttacks(mergedState, targetClone);
   ...
   * proximity to the attacker.
   *
   * @returns true if the target attack should be preferred, false otherwise.
   */
  private resolveProjectileAmbiguousAttack<T extends { type: number }>(
    merged: TickState,
    mergedAttack: WithProvenance<T>,
    target: TickState,
    targetAttack: WithProvenance<T>,
    attacker: string,
  ...
  /**
   * Merges NPC attack observations between merged and target tick states.
   */
  private mergeNpcAttacks(merged: TickState, target: TickState): void {
    for (const [roomId, baseNpc] of merged.getNpcs()) {
      const baseAttack = baseNpc.attack;
      const otherAttack = target.getNpcs().get(roomId)?.attack ?? null;
      ...
      if (baseAttack === null) {
        merged.setNpcAttack(roomId, otherAttack);
        continue;
      }
      ...
      if (baseAttack.type !== otherAttack.type) {
        if (!areProjectileAmbiguous(baseAttack.type, otherAttack.type)) {
          ...
          logger.warn('consolidate_npc_attack_type_mismatch', {
            tick: merged.getTick(),
            roomId,
            npcId: baseNpc.id,
            baseAttackType: baseAttack.type,
  ...
  /**
   * Reconciles two NPC attacks' targets, returning the merged attack.
   */
  private reconcileNpcAttackTarget(
    winner: WithProvenance<NpcAttacked>,
    loser: WithProvenance<NpcAttacked>,
    tick: number,
    roomId: number,
    npcId: number,
  ): WithProvenance<NpcAttacked> {
    if (winner.target === null) {
      return loser.target !== null
        ? { ...winner, target: loser.target }
        : winner;
    }

    if (loser.target !== null && winner.target !== loser.target) {
      this.qualityFlags.push({
        kind: 'NPC_ATTACK_TARGET_MISMATCH',
        tick,
        roomId,
        npcId,
        keptTarget: winner.target,
        discardedTarget: loser.target,
        keptSourceClientId: winner.sourceClientId,
        discardedSourceClientId: loser.sourceClientId,
      });
      logger.warn('consolidate_npc_attack_target_mismatch', {
        tick,
        roomId,
        npcId,
        keptTarget: winner.target,
  ...
  private remapToMergedSpace(): void {
    ...
    for (let i = 0; i < this.mergedTicks.length; i++) {
      ...
      for (const tagged of tickState.getTaggedEvents()) {
        ...
        const sourceTick = tagged.event.getTick();
        const offset = i - sourceTick;

        const remapped = remapEventTick(tagged, (tick) => {
          ...
          // with the offset between the event's source tick and its final
          // merged position. This preserves the relative distance but can be
          // wrong if insertions exist between the two ticks.
          const resolved = tick + offset;

          logger.warn('unmapped_cross_tick_reference', {
            eventType: tagged.event.getType(),
            source: tagged.source,
            clientTick: sourceTick,
            crossTickRef: tick,
            mergedTick: i,
            offset,
        ...
        remapped.event.setTick(i);
        remappedEvents.push(remapped);
      }
