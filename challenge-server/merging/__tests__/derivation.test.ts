    ...
    it('emits a consolidated event at the maze-end tick with unioned pivots', () => {
      ...
      const ctx = soteCtx(
        new Map<number, RegisteredClient>([
          [1, { client: clientA, status: MergeClientStatus.MERGED }],
          [2, { client: clientB, status: MergeClientStatus.MERGED }],
        ]),
      );
    ...
    it('skips unmerged clients', () => {
      ...
      mergeStageData(
        soteCtx(
          new Map<number, RegisteredClient>([
            [1, { client: clientA, status: MergeClientStatus.MERGED }],
            [2, { client: clientB, status: MergeClientStatus.UNMERGED }],
          ]),
        ),
        ticks,
      );
