...
describe('SimilarityScorer', () => {
  ...
  describe('player attack scoring', () => {
    ...
    it('does not penalize projectile-ambiguous player attacks', () => {
      const npcEvent = createNpcSpawnEvent({
        tick: 1,
        roomId: 5,
        npcId: 100,
      ...
      const ambiguousScore = new SimilarityScorer().score(base, target);
      const noAttackScore = new SimilarityScorer().score(
        noAttackBase,
        noAttackTarget,
      );

      expect(ambiguousScore).toEqual(noAttackScore);
    });

    it('penalizes projectile-ambiguous attacks with contradictory targets', () => {
      const npcSpawn1 = createNpcSpawnEvent({
        tick: 1,
        roomId: 5,
        npcId: 100,
        x: 10,
      ...
      const npcSpawn2 = createNpcSpawnEvent({
        tick: 1,
        roomId: 6,
        npcId: 101,
        x: 15,
      ...
      const base = createTickState(1, [basePlayer], [npcSpawn1, npcSpawn2]);
      const target = createTickState(
        1,
        [targetPlayer],
      ...
      const contradictoryScore = new SimilarityScorer().score(base, target);
      const noAttackScore = new SimilarityScorer().score(
        noAttackBase,
        noAttackTarget,
      );

      expect(contradictoryScore).toBeLessThan(noAttackScore);
    });

    it('does not penalize projectile-ambiguous attacks with unresolved targets', () => {
      const npcEvent = createNpcSpawnEvent({
        tick: 1,
        roomId: 5,
        npcId: 100,
  ...
  describe('NPC attack scoring', () => {
    it('does not penalize projectile-ambiguous NPC attacks', () => {
      const npcSpawn = createNpcSpawnEvent({
        ...
        x: 10,
        y: 10,
        hitpointsCurrent: 4000,
      });
      const baseAttack = createNpcAttackEvent({
        tick: 1,
        roomId: 1,
        npcId: 8388,
        attackType: NpcAttack.TOB_SOTE_BALL,
        target: 'player1',
      });
      const targetAttack = createNpcAttackEvent({
        ...
        roomId: 1,
        npcId: 8388,
        attackType: NpcAttack.TOB_SOTE_DEATH_BALL,
        target: 'player1',
      });
