...
const enum AttackComparison {
  MATCH,
  AMBIGUOUS_PROJECTILE,
  MISMATCH,
}

function compareAttacks(a: Attack, b: Attack): AttackComparison {
  if (a.secondaryId !== b.secondaryId) {
    return AttackComparison.MISMATCH;
  }
  if (a.typeId === b.typeId) {
    return AttackComparison.MATCH;
  }
  if (normalizeAttackType(a.typeId) === normalizeAttackType(b.typeId)) {
 ...
 *
 * @param npcId
 * @returns True if the NPC's hitpoints are exposed via a varbit.
 */
...
export class SimilarityScorer {
  ...
  private scoreAttacks(
    ...
    for (const [a, b] of inBoth) {
      const comparison = compareAttacks(a, b);
      switch (comparison) {
        case AttackComparison.MATCH:
          // Proceed to score the attack.
          break;
        case AttackComparison.AMBIGUOUS_PROJECTILE:
          // Ambiguous attacks are not scored, but a contradictory target is
          // still penalized.
          if (a.target !== null && b.target !== null && a.target !== b.target) {
            score += attackScores.contradictory;
          }
          continue;
        case AttackComparison.MISMATCH:
          score += attackScores.contradictory;
          continue;
      }
