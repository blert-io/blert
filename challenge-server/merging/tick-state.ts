...
export class TickState {
  ...
  /**
   * Removes an exact event object from this tick while preserving all other
   * finalized event state.
   * @param event Event object to remove.
   * @returns Whether the event was present.
   */
  public removeEvent(event: Event): boolean {
    const tagged = this.eventsByType.get(event.getType());
    if (tagged === undefined) {
      return false;
    }

    const index = tagged.findIndex((t) => t.event === event);
    if (index === -1) {
      return false;
    }
    ...
    if (tagged.length === 0) {
      this.eventsByType.delete(event.getType());
    }
    return true;
  }
  ...
  private createPlayerStateEvents(
    ...
    // Recreated equipment is always a delta instead of a snapshot as it's
    // built from internally consistent state.
    ...
    if (state.attack !== null) {
      ...
      if (state.attack.weaponId > 0) {
        attackEvent.playerAttack!.weapon = {
