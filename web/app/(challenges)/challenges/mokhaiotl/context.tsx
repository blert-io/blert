...
import { MokhaiotlChallenge } from '@blert/common';
...
export function MokhaiotlContextProvider({
  ...
  useEffect(() => {
    const loadMokhaiotl = async () => {
      setLoading(!challengeLoaded || challengeIdRef.current !== challengeId);
      try {
        const response = await fetch(
          `/api/v1/challenges/mokhaiotl/${challengeId}`,
        );
        if (response.status === 404) {
          setChallenge(null);
          return;
        }
        const challengeResponse = (await response.json()) as MokhaiotlChallenge;
    ...
    // Reload challenge every time the page changes to support in-progress challenges.
