...
function parseNpcActorId(actorId: string): number | null {
  return isNpcActorId(actorId) ? extractNpcRoomId(actorId) : null;
}
...
function isBlunder(
  ...
  const roomId = parseNpcActorId(action.targetActorId);
  if (roomId === null) {
    return false;
  }
...
export function BossPageAttackTimeline(props: BossPageAttackTimelineProps) {
  ...
  const normalizedParty = useMemo(() => {
    return new Map(
      props.playerState
        .keys()
        .map((name) => [toBcfNormalizedPlayerName(name), name]),
    );
  }, [props.playerState]);
